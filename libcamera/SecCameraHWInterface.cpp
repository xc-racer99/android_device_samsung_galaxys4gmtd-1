...
int CameraHardwareSec::pictureThread()
{
    ...
    if (mSecCamera->getCameraId() == SecCamera::CAMERA_ID_BACK) {
        ...
        memcpy(JpegHeap->base(), jpeg_data, jpeg_size);	
...
PostviewOverlayEnd:
    ...
    if (mMsgEnabled & CAMERA_MSG_COMPRESSED_IMAGE) {
        if (mSecCamera->getCameraId() == SecCamera::CAMERA_ID_BACK) {
            ...
            unsigned char *ImageStart = (unsigned char *)JpegHeap->base();
            sp<MemoryBase> mem = new MemoryBase(JpegHeap, 0, JpegImageSize);
            memcpy(ImageStart, JpegHeap->base(), JpegImageSize);
