...
def FullOTA_Assertions(info):
  ...
  info.output_zip.write(os.path.join(TARGET_DIR, "post-install.sh"), "post-install.sh")
  ...
  info.script.AppendExtra(
        ('package_extract_file("modem.bin.telusgalaxys4gmtd", "/tmp/modem.bin.telusgalaxys4gmtd");\n'
         'set_metadata("/tmp/modem.bin.telusgalaxys4gmtd", "uid", 0, "gid", 0, "mode", 0777);'))
  info.script.AppendExtra(
        ('package_extract_file("post-install.sh", "/tmp/post-install.sh");\n'
         'set_metadata("/tmp/post-install.sh", "uid", 0, "gid", 0, "mode", 0777);'))
  info.script.AppendExtra(
